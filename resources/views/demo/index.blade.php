...
<div class="banner">虚构演示 / 非生产数据 · LOCAL ONLY · 不连接 Google Sheet、生产库、OTA 或 WordPress</div>
<main class="wrap">
<h1>BoatOps 本地测试站</h1><p><strong>{{ $organization->name }}</strong> · 排期时区：<strong>{{ $organization->timezone }}</strong> · 未来 7 天：{{ $localStart->format('Y-m-d') }} 至 {{ $localEnd->subDay()->format('Y-m-d') }}</p>
...
<section class="card"><h2>库存 PURCHASE / LOAD / CONSUME / RETURN / WASTE</h2><form method="post" action="{{ route('demo.stock') }}" id="stock-form">@csrf<input type="hidden" name="command_id" value="{{ old('command_id', $commandIds['stock']) }}"><div class="grid">
...
<label>发生时间<input type="datetime-local" name="occurred_at" value="{{ old('occurred_at', $localNow->format('Y-m-d\TH:i')) }}" required></label><label>原因（WASTE 必填）<input name="reason"></label></div><p id="stock-hint" class="muted"></p><button>保存虚构库存流水</button></form></section>
...
<script>const type=document.getElementById("movement-type"),hint=document.getElementById("stock-hint");function explain(){hint.textContent={PURCHASE:"入仓；需账户和采购总成本，不选船/Trip。",LOAD:"从仓库装船；必须选船。",CONSUME:"从船上消耗；必须选船。",RETURN:"从船退回仓库；必须选船。",WASTE:"从仓库或船报损；必须填写原因。"}[type.value]}type.addEventListener("change",explain);explain();</script>
</body></html>
