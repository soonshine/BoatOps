...
final class InquiryController extends Controller
{
    ...
    {
        $i = $r->validate(['idempotency_key' => ['required', 'uuid'], 'reference' => ['required', 'string', 'max:100', 'regex:/^(?:FICTIONAL|TEST|DEMO)-[A-Z0-9][A-Z0-9_-]*$/'], 'boat_id' => ['nullable', 'integer', 'min:1'], 'trip_template_id' => ['nullable', 'integer', 'min:1'], 'slot_offering_id' => ['nullable', 'integer', 'min:1'], 'service_date' => ['nullable', 'date_format:Y-m-d'], 'notes' => ['nullable', 'string', 'max:1000']]);
        $o = $r->attributes->get('organization');
        ...
        $body = DB::transaction(function () use ($o, $payload, $hash, $key) {
            ...
            if ($e) {
                ...
                return json_decode($e->response_body, true, 512, JSON_THROW_ON_ERROR);
            }
            ...
            DB::table('idempotency_keys')->insert(['organization_id' => $o->id, 'operation' => self::OP, 'idempotency_key' => $key, 'request_hash' => $hash, 'response_status' => 201, 'response_body' => json_encode($body, JSON_THROW_ON_ERROR), 'created_at' => $now, 'updated_at' => $now]);

            return $body;
        }, 3);

        return redirect()->route('operator.inquiries.show', $body['inquiry_id']);
    }
